...
class TestText {
  public static createTestText(text: string): HTMLElement {
    const testTextWrapper = CreateElement.createElement(Tag.div, [
      { name: 'class', value: 'test-text-wrapper' },
    ]);
    const spanArray: Array<HTMLElement> = [];
    const testText = CreateElement.createElement(Tag.div, [
      { name: 'class', value: 'test-text' },
    ]);

    text.split('').forEach((char) => {
      const charSpan = CreateElement.createElement(Tag.span, [
        { name: 'class', value: 'char' },
      ]);
